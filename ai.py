...
piecePosScores = {
    'N': knightScore, 'B': bishopScore, 'Q': queenScore, 'R': rookScore,
    "wp": whitePawnScore, "bp": blackPawnScore
}
...
def order_moves(moves, board_obj, color):
    scored = []
    for move in moves:
        temp_board = safe_deepcopy(board_obj)
    ...
    if maximizing:
        ...
        for move in valid_moves:
            try:
                ...
                print(f"[!] Skipping move {move} due to error: {e}")
                continue
...
def is_valid_move(move):
    return is_valid_pos(move[0]) and is_valid_pos(move[1])
