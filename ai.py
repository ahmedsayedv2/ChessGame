...
def order_moves(moves, board_obj, color): 
    ...
    for move in moves: 
        ...
        # If we're giving up a low-value piece to capture high-value — good
        # If we're giving up high-value for low — bad
        if captured_val:
            move_value += (captured_val - moved_val)

        # Reward sacrifices if evaluation improves (e.g. delivering check)
    ...
    for r in range(8): 
        for c in range(8): 
            ...
            if piece.strip() and piece != '##': 
                ...
                # Use piece-square tables correctly
                key = piece_type
                if piece_type == "P":
                    key = piece.lower()  # "wp" or "bp"
